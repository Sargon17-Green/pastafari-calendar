...
const REPORT_PATH = path.join(ARTIFACT_DIR, "update-10c-calendrica-chinese-port-report.md");
const JSON_PATH = path.join(ARTIFACT_DIR, "update-10c-calendrica-chinese-port.json");
const SHA_PATH = path.join(ARTIFACT_DIR, "update-10c-calendrica-chinese-port-sha256sums.txt");
...
function mismatchList(actual, expected) {
  return Object.keys(expected)
    .filter((key) => actual[key] !== expected[key])
    .map((key) => ({ field: key, actual: actual[key], expected: expected[key] }));
}
...
function runFoundationDiagnostics() {
  ...
  const reverseExpected = fixedFromChinese(FOUNDATION_EXPECTED);
  const neighbors = [-2, -1, 0, 1, 2].map((offset) => ({
    offset,
    fixedDay: FOUNDATION + offset,
    chinese: comparableChinese(chineseFromFixed(FOUNDATION + offset)),
  }));
  const sampledWindow = [];
  for (let offset = -1000; offset <= 1000; offset += 100) {
    sampledWindow.push({ offset, fixedDay: FOUNDATION + offset, chinese: comparableChinese(chineseFromFixed(FOUNDATION + offset)) });
  }
  return {
    foundationActual,
    foundationExpected: FOUNDATION_EXPECTED,
    foundationMatch: sameChinese(foundationActual, FOUNDATION_EXPECTED),
    mismatches: mismatchList(foundationActual, FOUNDATION_EXPECTED),
    reverseExpected,
    reverseExpectedDeltaFromFoundation: reverseExpected - FOUNDATION,
    neighbors,
    sampledWindow,
  };
}
...
async function main() {
  ...
  const diagnostics = runFoundationDiagnostics();
  const result = {
    result: diagnostics.foundationMatch
      ? "CALENDRICA_PORT_REPRODUCES_FOUNDATION_READY_FOR_10D"
      : "CALENDRICA_PORT_DIVERGES_FROM_MAGILLAH_FOUNDATION",
    acceptanceStatus: diagnostics.foundationMatch
      ? "READY_FOR_PRODUCTION_SPAGHETTI_WRAPPER_STAGE_10D"
      : "NOT_ACCEPTED_AS_UPDATE_10_REPAIR",
    stage: "Update 10C",
    scope: "diagnostic/reference port only; production converter and public API unchanged",
    packageVersion: pkg.version,
    sources: {
      copyableImplementation: "CALENDRICA 4.0 / calendar-code2 / Apache-2.0",
      officialNormativeReference: "GB/T 33661-2017 — 农历的编算和颁行",
      repositoryChineseSourceText: "sources/chinese/农历规范算法.zh.md",
    },
    foundation: {
      fixedDay: FOUNDATION,
      jdn: FOUNDATION + RD_JDN_OFFSET,
      expected: FOUNDATION_EXPECTED,
    },
    diagnostics,
    conclusion: diagnostics.foundationMatch
      ? [
        "The independent non-Intl port now reproduces the Magillah Foundation discriminator.",
        "Production must still remain untouched until a separate Stage 10D spaghetti wrapper integrates the arithmetic shadow path while preserving the legacy Intl route.",
      ]
      : [
  ...
  const report = `# Update 10C — CALENDRICA Chinese port Foundation discriminator\n\n` +
    `Result: **${result.result}**\n\n` +
    `Acceptance: **${result.acceptanceStatus}**\n\n` +
    `## Scope\n\n` +
    `${result.scope}. No production file or public API path is modified by this stage.\n\n` +
    `## Sources\n\n` +
    `- Copyable implementation source: ${result.sources.copyableImplementation}.\n` +
    `- Official normative reference: ${result.sources.officialNormativeReference}.\n` +
    `- Repository Chinese source text: \`${result.sources.repositoryChineseSourceText}\`.\n\n` +
    `## Foundation discriminator\n\n` +
    `- Foundation fixed day: \`${FOUNDATION}\`\n` +
    `- Foundation JDN: \`${FOUNDATION + RD_JDN_OFFSET}\`\n` +
    `- Magillah expected: \`${JSON.stringify(FOUNDATION_EXPECTED)}\`\n` +
    `- Port actual: \`${JSON.stringify(diagnostics.foundationActual)}\`\n` +
    `- match: \`${diagnostics.foundationMatch}\`\n` +
    `- mismatches: \`${JSON.stringify(diagnostics.mismatches)}\`\n` +
    `- fixed-from-expected result: \`${diagnostics.reverseExpected}\`\n` +
    `- fixed-from-expected delta from Foundation: \`${diagnostics.reverseExpectedDeltaFromFoundation}\`\n\n` +
    `## Neighboring days\n\n` +
    diagnostics.neighbors.map((row) => `- offset ${row.offset}: fixed ${row.fixedDay} -> \`${JSON.stringify(row.chinese)}\``).join("\n") +
    `\n\n## Decision\n\n` +
    result.conclusion.map((line) => `- ${line}`).join("\n") +
    `\n`;
  ...
  for (const relative of [
    ...
    "artifacts/update-10c-calendrica-chinese-port.json",
    "artifacts/update-10c-calendrica-chinese-port-report.md",
  ]) {
    shaLines.push(`${await hashFile(relative)}  ./${relative}`);
  }
  await writeFile(SHA_PATH, `${shaLines.join("\n")}\n`);

  console.log(`[update10c] ${result.result}`);
  console.log(`[update10c] foundationMatch=${diagnostics.foundationMatch}`);
  console.log(`[update10c] report=${path.relative(ROOT, REPORT_PATH)}`);
}

main().catch((error) => {
  console.error(`[update10c] FAIL\n${error?.stack ?? error}`);
  process.exitCode = 1;
});
