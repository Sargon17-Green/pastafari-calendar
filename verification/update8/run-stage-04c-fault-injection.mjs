...
const OUT = path.join(ROOT, 'artifacts', 'update-08-stage-04c-fault-injection.json');
...
const HOOK = '__PASTAFARI_STAGE4C_FAULT__';

class Stage4CInjectedFault extends Error {
  constructor(checkpoint, meta = {}) {
    super(`STAGE4C_INJECTED_FAULT:${checkpoint}`);
    this.name = 'Stage4CInjectedFault';
...
const control = {
  ...
  hit(id, meta = {}) {
    ...
    if (this.enabled === id) throw new Stage4CInjectedFault(id, meta);
  },
...
function transformMain(body, params) {
  ...
  const cleanupNeedle=`${arenaParam}.length=MichalKorybut_PrzynaleznoscDynastycznaZmienialaSieCoAkapitBezZmianyRodzicowAniDatyUrodzenia_mechanizm_9da3b7046be5_n6;`;
  wrapper=replaceOnce(wrapper,cleanupNeedle,`${hookStmt('GEN_WRAP_BEFORE_CLEANUP')}${cleanupNeedle}${hookStmt('GEN_WRAP_AFTER_CLEANUP')}`);
...
function transformDecodedFunctionArgs(args) {
  ...
  const out=[...args]; let body=String(out[out.length-1]); const params=out.slice(0,-1).map(String);
  ...
  if(kind) transformedBodies.push({kind,originalLength:String(out[out.length-1]).length,transformedLength:body.length,originalSha256:originalHash,transformedSha256:sha256Text(body)});
...
try {
  ...
  publicApi=await import(`${pathToFileURL(PUBLIC_API).href}?stage4c_public=${Date.now()}`);
} finally {
  globalThis.Function=OriginalFunction; OriginalFunction.prototype.constructor=originalCtor;
...
const stage1Baseline=JSON.parse(await fs.readFile(path.join(ROOT,'verification','update8','stage-01-baseline.json'),'utf8'));
const sanityVector=stage1Baseline.canonicalSuccessVectors[0];
const semanticCalendar=new publicApi.PastafariCalendar({todayProvider:()=>null});
function normalizeTuple(r){ return {year:r.year.toString(),cutletName:r.cutletName,dayInCutlet:r.dayInCutlet,monthName:r.monthName,dayInMonth:r.dayInMonth}; }
function runSemanticSanity(){
  const actual=normalizeTuple(semanticCalendar.convertJdn(BigInt(sanityVector.input.targetJdn),{calculationJdn:BigInt(sanityVector.input.calculationJdn)}));
  return {id:sanityVector.id,input:sanityVector.input,expected:sanityVector.expected,actual,authoritativeEqualsReference:JSON.stringify(actual)===JSON.stringify(sanityVector.expected)};
}
control.disarm();
...
async function runCase({constructionId,symbol,checkpoint,argsFactory=()=>validArgs[symbol],stateAlreadyMutated,category,repeat=1}){
  for(let r=1;r<=repeat;r++){
    ...
    try{ ctorCall(symbol,args)(); returned=true; }catch(e){ observed={name:e.name,message:e.message,checkpoint:e.checkpoint??null,isInjected:e instanceof Stage4CInjectedFault || String(e.message).includes('STAGE4C_INJECTED_FAULT')}; }
    control.disarm();
...
const artifact={
  schema:'pastafari.update8.stage04c.fault-injection.v1',stage:'4C',generatedAt:new Date().toISOString(),
  revision:{repository:'Sargon17-Green/pastafari-calendar',branch:'main',campaignBaseCommit:'c141a94840a7806c73a8aaf8a87db22b7a9af1bc',packageVersion:'1.3.0',workingTree:'uploaded archive has no .git; analysis-only test script/artifacts added; production files unchanged'},
  alignment:{stage1Present:true,stage2aPresent:true,stage2bPresent:true,stage3Present:false,stage3Status:'MISSING: strict Stage-3 SHA alignment cannot be certified',productionCoreSha256:sha256Text(await fs.readFile(RAW_CORE,'utf8'))},
  injectionMechanism:{kind:'test-only in-memory decoded-Function source instrumentation',optIn:true,normalProductionFilesModified:false,publicApiAdded:false,uniqueException:'Stage4CInjectedFault / STAGE4C_INJECTED_FAULT:<checkpoint>',disabledMode:'hook records checkpoints but never throws; decoded production logic otherwise executes unchanged',transformedBodies},
  ...
  limitations:[
    'Stage 3 artifact/harness is absent from the supplied archive and current verification/update8 evidence available at campaign start; formal prerequisite alignment is unresolved.',
    'No constructor-time registry insertion was found in the Stage-2B inventory; therefore no artificial registry checkpoint was invented.',
    'No constructor-time semantic cache.set publication occurs in the four deeply instrumented constructors; PastafariCalendar cache allocations were tested, while conversion-time cache transaction testing belongs to 4D/existing cache-epoch tests.',
    'No global Math.random monkey patch was used; generated random/witness consumption is treated as confounding churn, not a semantic rollback oracle.',
    'Generated Function compilation occurs during module bootstrap rather than inside the selected constructor lifecycles; no fake constructor-time compile failure was claimed.'
  ],
  ...
  stageConclusion:{
    ...
    reason:'Fault-injection coverage is technically complete for the selected central lifecycles and confirms cleanup gaps, but the explicit Stage-3 artifact/SHA-alignment prerequisite is unavailable; Stage 5 must also wait for unified 4A-4D.'
  }
};
