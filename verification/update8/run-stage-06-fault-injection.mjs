...
const OUT = path.join(ROOT, 'artifacts', 'update-08-stage-06-fault-injection.json');
...
const HOOK = '__PASTAFARI_STAGE6_FAULT__';

class Stage6InjectedFault extends Error {
  constructor(checkpoint, meta = {}) {
    super(`STAGE6_INJECTED_FAULT:${checkpoint}`);
    this.name = 'Stage6InjectedFault';
...
const control = {
  ...
  hit(id, meta = {}) {
    ...
    if (this.enabled === id) throw new Stage6InjectedFault(id, meta);
  },
...
async function runCase({constructionId,symbol,checkpoint,argsFactory=()=>validArgs[symbol],stateAlreadyMutated,category,repeat=1}){
  for(let r=1;r<=repeat;r++){
    ...
    try{ ctorCall(symbol,args)(); returned=true; }catch(e){ observed={name:e.name,message:e.message,checkpoint:e.checkpoint??null,isInjected:e instanceof Stage6InjectedFault || String(e.message).includes('STAGE6_INJECTED_FAULT')}; }
    control.disarm();
...
// Stage 6 repeated injected-failure campaigns: prove immediate rollback does not accumulate.
await runCase({constructionId:'CTOR:authoritative:GregorianDate',symbol:'GregorianDate',checkpoint:'GEN_WRAP_AFTER_RESERVATION',stateAlreadyMutated:true,category:'STAGE6_REPEAT_100',repeat:100});
await runCase({constructionId:'CTOR:authoritative:GregorianDate',symbol:'GregorianDate',checkpoint:'GEN_WRAP_BEFORE_CLEANUP',stateAlreadyMutated:true,category:'STAGE6_REPEAT_100',repeat:100});
const stage6RepeatOptions={todayProvider:()=>null};
await runCase({constructionId:'CTOR:authoritative:PastafariCalendar',symbol:'PastafariCalendar',checkpoint:'GEN_RESERVE_AFTER_ARGUMENTS',stateAlreadyMutated:true,category:'STAGE6_REPEAT_100',argsFactory:()=>[stage6RepeatOptions],repeat:100});
...
const artifact={
  schema:'pastafari.update8.stage06.fault-injection.v1',stage:'6',generatedAt:new Date().toISOString(),
  ...
  injectionMechanism:{kind:'test-only in-memory decoded-Function source instrumentation',optIn:true,normalProductionFilesModified:false,publicApiAdded:false,uniqueException:'Stage6InjectedFault / STAGE6_INJECTED_FAULT:<checkpoint>',disabledMode:'hook records checkpoints but never throws; decoded production logic otherwise executes unchanged',transformedBodies},
  ...
  repeatedFailureSummary:coverage.filter(x=>x.checkpoint==='GEN_WRAP_AFTER_RESERVATION'||x.checkpoint==='GEN_WRAP_BEFORE_CLEANUP'||(x.constructionId.includes('PastafariCalendar')&&x.checkpoint==='GEN_RESERVE_AFTER_ARGUMENTS')),
  stage6RepeatedInjected:{
    checkpoints:['GEN_WRAP_AFTER_RESERVATION','GEN_WRAP_BEFORE_CLEANUP','GEN_RESERVE_AFTER_ARGUMENTS'],
    repeat100AllRestored:cases.filter(c=>c.category==='STAGE6_REPEAT_100').length===300 && cases.filter(c=>c.category==='STAGE6_REPEAT_100').every(c=>c.stateRestored),
    cases:cases.filter(c=>c.category==='STAGE6_REPEAT_100').map(c=>({constructionId:c.constructionId,checkpoint:c.checkpoint,repeat:c.repeat,stateRestored:c.stateRestored,arenaDelta:c.diffs.arenaDelta}))
  },
