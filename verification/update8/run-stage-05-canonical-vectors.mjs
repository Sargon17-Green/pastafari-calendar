...
import { PastafariCalendar, GregorianDate } from '../../src/public-api.js';
...
const results = vectors.map((vector) => {
  const actual = tuple(calendar.convertJdn(BigInt(vector.input.targetJdn), { calculationJdn: BigInt(vector.input.calculationJdn) }));
  const expected = {
  ...
  return { id: vector.id, input: vector.input, expected, actual, pass: true };
});

const artifact = {
  schema: 'pastafari.update8.stage05.canonical-vectors.v1',
  ...
  sourceOfExpectedValues: 'verification/update8/stage-01-baseline.json',
  vectors: results,
  result: 'PASS',
};
const out = path.join(ROOT, 'artifacts', 'update-08-stage-05-canonical-vectors.json');
fs.writeFileSync(out, `${JSON.stringify(artifact, null, 2)}\n`);
console.log(JSON.stringify({ result: 'PASS', vectors: results.map(({ id, actual }) => ({ id, actual })), output: out }, null, 2));
