...
const OUT_JSON = path.join(ROOT, "artifacts", "update-08-stage-04a-history.json");
const OUT_REPORT = path.join(ROOT, "artifacts", "update-08-stage-04a-report.md");
const OUT_SHA = path.join(ROOT, "artifacts", "update-08-stage-04a-sha256sums.txt");
...
function spawnChild(kind, payload = {}) {
  ...
  const child = spawnSync(
    ...
    ["--expose-gc", SCRIPT, "--child", kind, encoded],
    {
      ...
      env: { ...process.env, PASTAFARI_STAGE4A_CHILD: "1" },
    },
  );
  if (child.status !== 0) {
    throw new Error(
  ...
  const marker = "__STAGE4A_CHILD_JSON__";
...
if (childIndex >= 0) {
  ...
  try {
    ...
    process.stdout.write(`__STAGE4A_CHILD_JSON__${JSON.stringify(serialize(result))}\n`);
  } catch (error) {
