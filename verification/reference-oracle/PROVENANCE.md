...
| Twelve post-pour stirs | 556–598 | At the start of each round form raw `bowlSum = sum(old bowls)`, add `149*round`, then preserve: `orderNumber = keep(bowlSum + 149*round)`. This preserved value is the Scroll’s “preserved sum of the bowls”: it both selects the permutation and is added to each bowl’s `u`. The raw `bowlSum` remains trace/provenance only. All six outputs use the same old snapshot and are committed together. |
