...
ה־PWA הציבורי מציג את אותו solver בשתי שכבות ממשק, אך שתיהן מגיעות בסופו
של דבר אל `solvePastafariConstraints()`:

1. **חיפוש תאריך יחיד** בונה משתנה target אחד ואילוץ Pastafari. יום המעשה
   יכול להיות היום הפעיל באתר, יום מוחלט שהומר ל־JDN דרך ממירי האתר, או
   `SAME_AS_TARGET` עם תחום סופי. תלות פסטפרית נוספת מועברת לעורך המתקדם.
2. **מערכת אילוצים** מאפשרת למשתמש להגדיר משתנים, תחומים ואת כל סוגי
   האילוצים המתועדים לעיל, לרבות strongly connected components מחזוריים.

ממשק האתר אינו מניח ייחודיות. הוא מציג את כל `solutions` שהוחזרו ואת מצב
`complete` בנפרד. תוצאה ריקה מוצגת כ״אין פתרון״ רק כאשר `complete === true`.
ב־`complete === false` היא מוצגת כחיפוש שלא הושלם.

אירועי progress מוצגים כיחידות עבודה ולא כאחוזים, משום ש־`total` הוא `null`.
גם `matches` אינו מוצג כמונה גלובלי של פתרונות סופיים; רק `solutions` אחרי
החזרת התוצאה משמשים להצגת פתרונות.

פתרון של חיפוש יחיד ניתן לפתיחה בלוח הרגיל באמצעות צמד ה־JDN המדויק
`(target, calculation)`. במערכת מרובת אילוצים אין יום מעשה גלובלי יחיד;
לכן פתיחה בלוח נעשית בהקשר של אילוץ Pastafari מסוים.

ה־Service Worker שומר offline את ה־constraint client, ה־solver, ה־reverse
worker ואת אותו מנוע קנוני מלא. בדיקת PWA מפעילה reverse גם לאחר כיבוי
שרת הבדיקה כדי לוודא שכל שרשרת ה־Worker זמינה מהמטמון.
